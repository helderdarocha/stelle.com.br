...
<html>
<head>
	<title>A Divina Comédia: Purgatório - Testes</title>
    <style>
    input {background: rgb(40%,60%,20%);
    ...
    <script>
	root = "/";
    doc = root + "pt/purgatorio/teste/index.jsp";
      
	if (parent.nav) {
		parent.nav.location.href=root+"pt/purgatorio/navbar.jsp";
	}
    
	if (!parent.comedia) {
        document.cookie="menu_imagem=" + escape(doc) + "; path="+root+"pt/purgatorio/";
		location.href=root+"pt/purgatorio/purgatorio.html";
	}
	</script>
    <style>
...
<tr><td align=left><A HREF="../index.html">Purgatório (página principal)</A></td>
<script>
   if (!parent.nav) {
    s = "<td class='minimenu'><a href='../purgatorio.html' target='_top' onclick=\"javascript:document.cookie='menu_imagem=' + doc\">Recuperar barra de navegação</a></td>";
	document.write(s);
   }
   </script>
</tr></table><hr><p></P>
<h1>Testes sobre o Purgatório
</h1>
<p class=primeiro>Fazendo os testes de múltipla escolha disponíveis neste <em>site</em>, você poderá testar seus conhecimentos sobre o <em>Purgatório</em> de Dante e recordar a sua viagem. 
...
<input type="Hidden" name="livro" value="purgatorio">
<input type="Hidden" name="docbase" value="/pt/purgatorio/">
<input type="Hidden" name="siteroot" value="/">
<input type="Hidden" name="pag_erro" value="/pt/purgatorio/teste/erro.html">
<input type="Hidden" name="redirect" value="/pt/purgatorio/teste/resposta.html">
...
<tr><td align=left><A HREF="../index.html">Purgatório (página principal)</A></td></tr></table><p></P>
</TD></TR></TABLE>
</body>
</html>
