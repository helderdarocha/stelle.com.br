<% 
	final String RAIZ = "/WEB-INF/questoes/purgatorio";
	int MAX = 30;
...
<td align=right width=33%><a title="Página do Purgatório" href="../index.html">Sair</a></td></tr></table>
</div>
<form>
<h1>A Divina Comédia: O Purgatório</h1>
...
<td align=right><input type=button value=" &lt;&lt;&lt; Teste No. <%=grupo - 1 %>" title="Retornar ao teste anterior"  style="background-color: rgb(0%,30%,0%)"
...
<% if (questao == fim) { %>
<td align=right width=33%><input type=submit title="Enviar este teste (questões <%=inicio %> a <%=fim %>)" value="E N V I A R" style="background-color: rgb(0%,70%,0%)"
onclick="mark('dante<%=grupo%>', '<%=questao %>', this.form.q, 255); enviar(this.form, 'dante<%=grupo%>')"></td>
	<% if (fim != MAX) { %>
<td align=right><input type=button value="Teste No. <%=grupo + 1 %> &gt;&gt;&gt;" title="Continuar (enviar respostas deste teste mais tarde)"  style="background-color: rgb(0%,30%,0%)"
onclick="mark('dante<%=grupo%>', '<%=questao %>', this.form.q, 1)"></td>
    <% } %>
<% } else if (questao != fim) { %>
